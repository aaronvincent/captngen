...
!! For mesa interface only: allocate arrays.
  subroutine allocate_stellar_arrays(nlines_mesa, opt_niso)
    ...
    nlines = nlines_mesa
    stellar_niso = 8
    if (present(opt_niso)) stellar_niso = opt_niso
    ...
    if (.not. allocated(tab_mfr_oper)) allocate(tab_mfr_oper(nlines,16))
    if (.not. allocated(vesc_shared_arr)) allocate(vesc_shared_arr(nlines))
  ...
  subroutine deallocate_stellar_arrays()
    ...
    deallocate(tab_dr)
    deallocate(tab_g)
    if (allocated(tab_mfr_oper)) deallocate(tab_mfr_oper)
    if (allocated(vesc_shared_arr)) deallocate(vesc_shared_arr)
    RETURN
