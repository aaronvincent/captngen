...
!----------------------------------------------------------------------------------------------------
    ...
    !SB: Trying to figure out the v-2 integrations (cannot be done analytically)
    !SB: This subroutine is to preform the "t" integral of A.26 for n = -2 after having done the "s" integral analytically
    !SB: Comparing these results with mathematica

    function diff_scattering_rate_vminus1(w, vesc)
        implicit none
        double precision :: w, diff_scattering_rate_vminus1, vesc
        double precision :: wNat, vescNat, ueNat, v0Nat
        ue = ue_at_r
        w = 146674483.49636036d0
        diff_scattering_rate_vminus1 = integral1_vminus(w, vesc) + &
                                      integral2_vminus(w, vesc)
        diff_scattering_rate_vminus1 = diff_scattering_rate_vminus1*&
                                  1./(2.*ue**2)*(1.+mu)**(3./2.)*v0**2
        ! print*, "mu: ", mu
        ! print*, "ue: ", ue
        ! print*, "w: ", w
        ! print*, "vesc: ", vesc
        ! print*, "prefactor: ", 1./(2.*ue**2)*(1.+mu)**(3./2.)*v0**2
        ! print*, "int1: ", integral1_vminus(w, vesc)
        ! print*, "int2: ", integral2_vminus(w, vesc)
        ! print*, "Int Tot: ", diff_scattering_rate_vminus1
        ! print*, "**********************"
    end function


    FUNCTION integrand1_vminus(NUMFUN,X) RESULT(Value)
      USE Precision_Model
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: NUMFUN
      REAL(kind=stnd), DIMENSION(:), INTENT(IN) :: X
      REAL(kind=stnd), DIMENSION(NUMFUN) :: Value
      REAL(kind=stnd) :: t, v, erfDiff
      t = x(1)
      v = x(2)


      Value(1) = (exp((mu*(-((1 + mu)*t**2) + w_shared**2))/ue**2)*v*&
                (erf((Sqrt(1 + mu)*(t + v))/ue) - &
                 erf((Sqrt(1 + mu)*(-t + w_shared))/ue)))/t

    END FUNCTION integrand1_vminus

    function integral1_vminus(w, vesc)
      USE Precision_Model
      ...
      double precision :: integral1_vminus
      double precision :: w, vesc
      ...
      Vertices(1:n,0,1) = (/w/2. , 0.d0 /)
      Vertices(1:n,1,1) = (/(w-vesc)/2., vesc /)
      Vertices(1:n,2,1) = (/(w+vesc)/2., vesc /)


      ! epsrel = 1.0e-4_stnd
       CALL CUBATR(n,l,integrand1_vminus,m,Vertices(:,:,1:m),RgType,IntegralValue, &
            AbsErr,Epsrel=epsrel,NEval=NEval,JOB=2,MaxPts=10000)
       CALL CUBATR()
       integral1_vminus = IntegralValue(1)
       ! print*, "Integral Result 1: ", integral1_vminus
    end function

    FUNCTION integrand2_vminus(NUMFUN,X) RESULT(Value)
      USE Precision_Model
      INTEGER, INTENT(IN) :: NUMFUN
      REAL(kind=stnd), DIMENSION(:), INTENT(IN) :: X
      REAL(kind=stnd), DIMENSION(NUMFUN) :: Value
      ...
      Value(1) = -((exp((mu*(-((1 + mu)*t**2) + w_shared**2))/ue**2)*v* &
                   (Erf((Sqrt(1 + mu)*(t - v))/ue) - &
                    Erf((Sqrt(1 + mu)*(t + v))/ue)))/t)

    END FUNCTION integrand2_vminus

    function integral2_vminus(w, vesc)
      USE Precision_Model
      USE CUI                      ! Cubpack User Interface
      IMPLICIT NONE
      INTEGER, PARAMETER :: n=2, & ! the dimension
      ...
      double precision :: integral2_vminus
      double precision :: w, vesc
      double precision :: cutoffIntegrand
      ...
      RgType(1) = Parallelogram

      cutoffIntegrand = 1d9

      Vertices(1:n,0,1) = (/w/2. , 0.d0 /)
      Vertices(1:n,1,1) = (/cutoffIntegrand, 0.d0 /)
      Vertices(1:n,2,1) = (/(w+vesc)/2. , vesc /)

      epsrel = 1.0e-6_stnd
       CALL CUBATR(n,l,integrand2_vminus,m,Vertices(:,:,1:m),RgType,IntegralValue, &
            AbsErr,Epsrel=epsrel,NEval=NEval,JOB=2,MaxPts=10000)
       CALL CUBATR()
       integral2_vminus = IntegralValue(1)
       ! print*, "Integral Result 2: ", integral2_vminus
    end function

    ! FUNCTION integrand3_vminus(NUMFUN,X) RESULT(Value)
    !   USE Precision_Model
    !   IMPLICIT NONE
    !   INTEGER, INTENT(IN) :: NUMFUN
    !   REAL(kind=stnd), DIMENSION(:), INTENT(IN) :: X
    ...
    !
    !
    !   Value(1) = (exp((mu*(-((1 + mu)*t**2)))/ue**2)*v* &
    !             Erf((Sqrt(1 + mu)*(t - v))/ue))/t
    !
    !   ! print*, "t: ", t
    !   ! print*, "v: ", v
    !   ! print*, "erf difference: ", (Erf((Sqrt(1 + mu)*(t + v))/ue) - &
    !   !         Erf((Sqrt(1 + mu)*(-t + w_shared))/ue))
    ! END FUNCTION integrand3_vminus
    !
    ! function integral3_vminus(w, vesc)
    !   USE Precision_Model
    !   USE CUI                      ! Cubpack User Interface
    !   IMPLICIT NONE
    !   INTEGER, PARAMETER :: n=2, & ! the dimension
    ...
    !   REAL(kind=stnd) :: epsrel
    !
    !   double precision :: integral3_vminus
    !   double precision :: w, vesc
    !
    !   w_shared = w
    !   ue = ue_at_r
    ...
    !
    !   Vertices(1:n,0,1) = (/w/2. , 0.d0 /)
    !   Vertices(1:n,1,1) = (/cutoffIntegrand , 0d0 /)
    !   Vertices(1:n,2,1) = (/(w+vesc)/2., vesc /)
    !
    !
    !   epsrel = 1.0e-6_stnd
    !    CALL CUBATR(n,l,integrand3_vminus,m,Vertices(:,:,1:m),RgType,IntegralValue, &
    !         AbsErr,Epsrel=epsrel,NEval=NEval,JOB=2,MaxPts=10000)
    !    CALL CUBATR()
    !    integral3_vminus = IntegralValue(1)
    !   print*, "Integral Result 3: ", integral3_vminus
    ! end function
