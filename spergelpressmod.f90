...
function nx_func(T_x, dTdr, mfp, sigma_N, sigma_0, alpha, kappa, Nwimps, niso)
implicit none
...
double precision :: n_0, fgoth, Tc, K, rchi, integrand, mxg, alphaofR(nlines), kappaofR(nlines)
double precision :: R(nlines), phi(nlines), dR(nlines), dphidr(nlines), n_nuc(niso,nlines)
double precision :: nx(nlines), nx_iso(nlines), nx_func(nlines), cumint(nlines), cumNx(nlines)
integer :: i, j
...
nx_func = nx_iso !fgoth*nx + (1.-fgoth)*nx_iso
...
double precision, intent(in) :: dTdr(nlines), mfp(nlines)
double precision, intent(in) :: alpha(niso), kappa(niso), sigma_N(niso)
double precision :: alphaofR(nlines), kappaofR(nlines), R(nlines), integrand(nlines)
...
function newtons_meth(f, dTdr, mfp, sigma_N, sigma_0, alpha, kappa, Nwimps, niso, guess_1, guess_2, tolerance)
...
double precision, intent(in) :: Nwimps, sigma_0, tolerance, guess_1, guess_2
double precision, intent(in) :: dTdr(nlines), mfp(nlines)
double precision, intent(in) :: alpha(niso), kappa(niso), sigma_N(niso)
...
do while (error > tolerance)
	...
	x_3 = x_2 - f(x_2, dTdr, mfp, sigma_N, sigma_0, alpha, kappa, Nwimps, niso)*(x_2-x_1) &
		/(f(x_2, dTdr, mfp, sigma_N, sigma_0, alpha, kappa, Nwimps, niso) - &
		f(x_1, dTdr, mfp, sigma_N, sigma_0, alpha, kappa, Nwimps, niso))
	error = abs(x_3-x_2)
	x_1 = x_2
	x_2 = x_3
enddo
