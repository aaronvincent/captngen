...
subroutine splinebc (x, y, b, c, d, n,bc)
!======================================================================
!  Calculate the coefficients b(i), c(i), and d(i), i=1,2,...,n
!  for cubic spline interpolation
!  s(x) = y(i) + b(i)*(x-x(i)) + c(i)*(x-x(i))**2 + d(i)*(x-x(i))**3
...
implicit none
integer n
double precision x(n), y(n), b(n), c(n), d(n), bc
integer i, j, gap
double precision h
...
function isplined1(x, b, c, d, n) !doesn't depend on 1
!======================================================================
! function isplined1 evaluates first derivative from
! the cubic spline interpolation at point z, at grid point x
! ispline = d/dx (b(i)*(u-x(i))+c(i)*(u-x(i))**2+d(i)*(u-x(i))**3)
! where  x(i) <= u <= x(i+1)
!----------------------------------------------------------------------
! input..
! u       = the abscissa at which the spline is to be evaluated
! x,      = the arrays of given data points
! b, c, d = arrays of spline coefficients computed by spline
! n       = the number of data points
! output:
! ispline = interpolated value at point u
...
implicit none
integer n
double precision isplined1(n)
double precision  u(n), x(n), b(n), c(n), d(n)
! integer i, j, k
double precision dx

! do i = 1,n
isplined1 = b + x*(2.*c + 3.*x*d)
! end do


end function isplined1
