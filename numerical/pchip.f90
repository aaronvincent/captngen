...
!*****************************************************************************80
!
!! d1machPC returns real ( kind = 8 ) real machine constants.
!
!  Discussion:
!
...
!    exponent, then
!
!      d1machPC(1) = B^(EMIN-1), the smallest positive magnitude.
!      d1machPC(2) = B^EMAX*(1-B^(-T)), the largest magnitude.
!      d1machPC(3) = B^(-T), the smallest relative spacing.
!      d1machPC(4) = B^(1-T), the largest relative spacing.
!      d1machPC(5) = log10(B).
!
!  Licensing:
!
...
!    1 <= I <= 5.
!
!    Output, real ( kind = 8 ) d1machPC, the value of the chosen parameter.
!
  implicit none

  real ( kind = 8 ) d1machPC
  integer ( kind = 4 ) i

  if ( i < 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'd1machPC - Fatal error!'
    ...
    d1machPC = 4.450147717014403D-308
  else if ( i == 2 ) then
    d1machPC = 8.988465674311579D+307
  else if ( i == 3 ) then
    d1machPC = 1.110223024625157D-016
  else if ( i == 4 ) then
    d1machPC = 2.220446049250313D-016
  else if ( i == 5 ) then
    d1machPC = 0.301029995663981D+000
  else if ( 5 < i ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'd1machPC - Fatal error!'
    write ( *, '(a)' ) '  The input argument I is out of bounds.'
...
!
  ...
  real ( kind = 8 )  D1, D2, DELTA, d1machPC
  integer ( kind = 4 ) ISMON, ITRUE
...
!
  ...
  integer ( kind = 4 ) LUN, KPRINT, IPASS
  real ( kind = 8 ) DBVALU, d1machPC
  EXTERNAL  DBVALU, DPCHBS, d1machPC
...
!*****************************************************************************80
!
!! fdumpPC can be used to print a symbolic dump.
!
!  Discussion:
!
!    fdumpPC is intended to be replaced by a locally written
...
!    xgetuaPC routine.  See XSETUA and xgetuaPC for details.
!
!  Modified:
!
!    05 April 2015
...
!*****************************************************************************80
!
!! i1machPC returns integer machine constants.
!
!  Discussion:
!
!    Input/output unit numbers.
!
!      i1machPC(1) = the standard input unit.
!      i1machPC(2) = the standard output unit.
!      i1machPC(3) = the standard punch unit.
!      i1machPC(4) = the standard error message unit.
!
!    Words.
!
!      i1machPC(5) = the number of bits per integer storage unit.
!      i1machPC(6) = the number of characters per integer storage unit.
!
!    Integers.
!
...
!    where 0 <= X(1:S-1) < A.
!
!      i1machPC(7) = A, the base.
!      i1machPC(8) = S, the number of base A digits.
!      i1machPC(9) = A^S-1, the largest integer.
!
...
!
!      i1machPC(10) = B, the base.
!
!    Single precision
!
!      i1machPC(11) = T, the number of base B digits.
!      i1machPC(12) = EMIN, the smallest exponent E.
!      i1machPC(13) = EMAX, the largest exponent E.
!
!    real ( kind = 8 )
!
!      i1machPC(14) = T, the number of base B digits.
!      i1machPC(15) = EMIN, the smallest exponent E.
!      i1machPC(16) = EMAX, the largest exponent E.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!    1 <= I <= 16.
!
!    Output, integer ( kind = 4 ) i1machPC, the value of the chosen parameter.
!
  implicit none

  integer ( kind = 4 ) i
  integer ( kind = 4 ) i1machPC

  if ( i < 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'i1machPC - Fatal error!'
    write ( *, '(a)' ) '  The input argument I is out of bounds.'
...
!
  ...
  RETURN
END
FUNCTION j4savePC ( IWHICH, IVALUE, ISET )

!*****************************************************************************80
!
!! j4savePC saves or recalls global variables needed by the error handler.
!
!  Discussion:
!
!    j4savePC saves and recalls several global variables needed
...
!        in the function value, j4savePC.
!
  implicit none
...
!
  ...
  LKNTRL = j4savePC (2, 0, .FALSE.)
  MAXMES = j4savePC (4, 0, .FALSE.)
!
...
!
  ...
  CALL xgetuaPC(IU,NUNIT)
!
!  A ZERO VALUE FOR A LOGICAL UNIT NUMBER MEANS TO USE THE STANDARD
!  ERROR MESSAGE UNIT INSTEAD.  i1machPC(4) RETRIEVES THE STANDARD
!  ERROR MESSAGE UNIT.
!
  N = i1machPC(4)
  DO I=1,NUNIT
...
!  Print to each unit.
!
     CALL xgetuaPC (LUN, NUNIT)

     DO KUNIT = 1,NUNIT
        IUNIT = LUN(KUNIT)
        IF (IUNIT == 0) IUNIT = i1machPC(4)
!
...
!
  ...
  KONTRL = j4savePC(2,0,.FALSE.)

  RETURN
END
SUBROUTINE xgetuaPC ( IUNITA, N )

!*****************************************************************************80
!
!! xgetuaPC returns error unit numbers.
!
!  Discussion:
!
!    xgetuaPC may be called to determine the unit number or numbers
...
!                default unit, as defined by the i1machPC machine
!                constant routine.  Only IUNIT(1),...,IUNIT(N) are
!                defined by xgetuaPC.  The values of IUNIT(N+1),...,
!                IUNIT(5) are not defined (for N .LT. 5) or altered
!                in any way by xgetuaPC.
!
...
!
  ...
  DO I=1,N
    ...
    IUNITA(I) = j4savePC(INDEX,0,.FALSE.)
  end do
