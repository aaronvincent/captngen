...
module nreo_mod
    ...
    subroutine init_rw_prefactors(spin_dm, all_prefactors)
        ...
        double precision :: response_dm_d, response_dm_m, response_dm_p2m, response_dm_p1, response_dm_p2, response_dm_s1
        double precision :: response_dm_ds1, response_dm_s2 ! DM response R functions stored in their own source files
                        ...
                        do term_w = 1, 7
                            ...
                            if ( nuclear_responses(func_type,eli,tau,tau_p,term_w) .ne. 0.d0 ) then
                                ...
                                do term_r = 1, 6
                                    ...
                                    case (1)
                                        r_const = response_dm_m(m_proton,c0,tau,tau_p,term_r-1,spin_dm,couplings_nreo) !!!!!!!!!!!!!!! in the DM response R functions the R term starts at zero, should change it to start at 1 like other Fortran things do for consistency
                                    case (2)
                                        r_const = response_dm_s2(m_proton,c0,tau,tau_p,term_r-1,spin_dm,couplings_nreo)
                                    case (3)
                                    ...
                                    case (5)
                                        r_const = response_dm_p2m(m_proton,tau,tau_p,term_r-1,spin_dm,couplings_nreo)
                                    case (6)
                                        r_const = response_dm_p1(m_proton,tau,tau_p,term_r-1,spin_dm,couplings_nreo)
                                    case (7)
                                        r_const = response_dm_d(m_proton,tau,tau_p,term_r-1,spin_dm,couplings_nreo)
                                    case (8)
                                        r_const = response_dm_ds1(tau,tau_p,term_r-1,spin_dm,couplings_nreo)
                                    case default
...
subroutine init_nreo()
    ...
    real :: response_nuc_m, response_nuc_s2, response_nuc_s1, response_nuc_p2, response_nuc_p2m, response_nuc_p1, response_nuc_d
    real :: response_nuc_ds1
        ...
        do i=1,16
            do j=1,2
                do k=1,2
                    do l=1,7
                        if (m.eq.1) then
                            nuclear_responses(m,i,j,k,l) = response_nuc_m(j-1,k-1,isotope_strings(i),terms(l))
                        else if (m.eq.2) then
                            ...
                            nuclear_responses(m,i,j,k,l) = response_nuc_p2(j-1,k-1,isotope_strings(i),terms(l))
                        else if (m.eq.5) then
                            nuclear_responses(m,i,j,k,l) = response_nuc_p2m(j-1,k-1,isotope_strings(i),terms(l))
                        else if (m.eq.6) then
                            nuclear_responses(m,i,j,k,l) = response_nuc_p1(j-1,k-1,isotope_strings(i),terms(l))
                        else if (m.eq.7) then
                            nuclear_responses(m,i,j,k,l) = response_nuc_d(j-1,k-1,isotope_strings(i),terms(l))
                        else
                            nuclear_responses(m,i,j,k,l) = response_nuc_ds1(j-1,k-1,isotope_strings(i),terms(l))
                        end if
                    end do
                end do
            end do
